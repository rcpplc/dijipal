...
const CategoryDetailPage = () => {
  ...
  const renderBreadcrumb = () => {
    if (locationSlug) {
      // Alt kategori breadcrumb
      return (
        <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-6">
          <Link to="/" className="hover:text-blue-600">Ana Sayfa</Link>
          <ChevronRight className="w-4 h-4" />
          <Link to="/kategoriler" className="hover:text-blue-600">Kategoriler</Link>
          <ChevronRight className="w-4 h-4" />
          <Link to={`/${categorySlug}`} className="hover:text-blue-600">{categoryData?.parent_category_title || categorySlug}</Link>
          <ChevronRight className="w-4 h-4" />
          <span className="text-gray-800 font-medium">{categoryData?.location_name}</span>
        </nav>
      );
    } else {
      // Ana kategori breadcrumb
      return (
        <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-6">
          <Link to="/" className="hover:text-blue-600">Ana Sayfa</Link>
          <ChevronRight className="w-4 h-4" />
